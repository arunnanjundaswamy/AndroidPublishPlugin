...
class PlayPublisherPlugin implements Plugin<Project> {
    ...
    void apply(Project project) {
        ...
        project.android.applicationVariants.all { variant ->
            ...
            def publishOnlyApkTaskName = "publishOnlyApk${variationName}"
            def publishListingTaskName = "publishListing${variationName}"
            def publishTaskName = "publish${variationName}"
            def promoteToBetaTaskName = "promoteToBeta${variationName}"
            def promoteToProdTaskName = "promoteToProd${variationName}"
            ...
            def promoteToBetaTask = project.tasks.create(promoteToBetaTaskName, PromoteToBetaTask)
            promoteToBetaTask.description = "Promotes APK in Alpha track of play store to Beta"
            promoteToBetaTask.extension = extension
            promoteToBetaTask.variant = variant
            promoteToBetaTask.group = PLAY_STORE_GROUP

            def promoteToProdTask = project.tasks.create(promoteToProdTaskName, PromoteToProdTask)
            promoteToProdTask.description = "Promotes APK in Beta track of play store to Production"
            promoteToProdTask.extension = extension
            promoteToProdTask.variant = variant
            promoteToProdTask.group = PLAY_STORE_GROUP
            ...
            if (zipAlignTask && variantData.zipAlignEnabled) {
                ...
                publishApkTask.description = "Builds and uploads the APK for the ${variationName} build"
                publishApkTask.group = PLAY_STORE_GROUP

                def publishOnlyApkTask = project.tasks.create(publishOnlyApkTaskName, PlayOnlyPublishApkTask)
                publishOnlyApkTask.extension = extension
                publishOnlyApkTask.variant = variant
                publishOnlyApkTask.inputFolder = playResourcesTask.outputFolder
                publishOnlyApkTask.description = "Uploads the APK for the ${variationName} build"
                publishOnlyApkTask.group = PLAY_STORE_GROUP
